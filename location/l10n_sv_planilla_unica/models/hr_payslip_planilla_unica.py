...
def _only_digits(s):
    """Devuelve solo dígitos; si s es None/'' → ''."""
    if not s:
        return ''
...
class HrPayslipPlanillaUnica(models.Model):
    ...
    afp_confia = constants.AFP_CONFIA
    afp_crecer = constants.AFP_CRECER
    afp_ipsfa = constants.AFP_IPSFA
    ...
    def _select(self):
        ...
        MAX(
            CASE
            WHEN COALESCE(NULLIF(c.afp_id, ''), '') = '{self.afp_confia}' THEN 'COF'
            WHEN COALESCE(NULLIF(c.afp_id, ''), '') = '{self.afp_crecer}' THEN 'MAX'
            ELSE NULL -- cuando es ipsfa se deja en blanco
            END
        ) AS afp_id,
